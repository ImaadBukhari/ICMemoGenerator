...
def generate_short_word_document(db: Session, memo_request_id: int) -> Optional[str]:
    """Generate a 1-page Word document with compact formatting"""
    ...
    try:
        ...
        # Create document
        print("Creating short Word document...")
        doc = Document()
        create_short_memo_styles(doc)
        
        # Add document title and company info
        title_para = doc.add_paragraph("INITIAL IC MEMO", style='Short Memo Title')
        company_para = doc.add_paragraph(memo_request.company_name, style='Short Company Name')
        ...
        # Add header and footer
        add_short_header_footer(doc, memo_request.company_name, generation_date)
        
        # Create sections lookup
        sections_dict = {section.section_name: section for section in sections}
        
        # Add Problem/Solution table first
        if "company_brief" in sections_dict:
            print("Adding problem/solution table...")
            create_problem_solution_table(doc, sections_dict["company_brief"].content)
            doc.add_paragraph()  # Spacing
        
        # Add Company Brief
        if "company_brief" in sections_dict:
            print("Adding company brief...")
            add_short_section_to_document(doc, "company_brief", sections_dict["company_brief"].content, 
                                        {"company_brief": "Company Brief"})
            doc.add_paragraph()  # Spacing
        
        # Add IC Blurb sections
        ic_sections = {
            "startup_overview": "Startup Overview, Product, and Model",
            "founder_team": "Founder Team", 
            "deal_traction": "Deal and Traction",
            "competitive_landscape": "Competitive Landscape",
            "remarks": "Remarks"
        }
        
        for section_key, section_title in ic_sections.items():
            if section_key in sections_dict:
                print(f"Adding IC section: {section_title}")
                add_short_section_to_document(doc, section_key, sections_dict[section_key].content, 
                                            {section_key: section_title})
                doc.add_paragraph()  # Spacing between sections
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        safe_company_name = memo_request.company_name.replace(' ', '_').replace('/', '_')
        filename = f"IC_Memo_Short_{safe_company_name}_{timestamp}.docx"
        file_path = os.path.join(docs_dir, filename)
        
        print(f"Saving short document to: {file_path}")
        
        # Save the document
        doc.save(file_path)
        print("✅ Short Word document saved successfully")
        
        # Verify file exists
        if os.path.exists(file_path):
            file_size = os.path.getsize(file_path)
            print(f"✅ Short document file exists, size: {file_size} bytes")
            return file_path
        else:
            print("❌ Short document file was not created")
            return None
...
def create_short_memo_styles(doc: Document):
    """Create custom styles for the short memo document"""
    ...
    # Short memo title style
    if 'Short Memo Title' not in [s.name for s in styles]:
        title_style = styles.add_style('Short Memo Title', WD_STYLE_TYPE.PARAGRAPH)
        title_font = title_style.font
        title_font.name = DOCUMENT_FONT
    ...
    # Short company name style
    if 'Short Company Name' not in [s.name for s in styles]:
        company_style = styles.add_style('Short Company Name', WD_STYLE_TYPE.PARAGRAPH)
        company_font = company_style.font
        company_font.name = DOCUMENT_FONT
    ...
    # Short section heading style
    if 'Short Section Heading' not in [s.name for s in styles]:
        heading_style = styles.add_style('Short Section Heading', WD_STYLE_TYPE.PARAGRAPH)
        heading_font = heading_style.font
        heading_font.name = DOCUMENT_FONT
    ...
    # Short body text style
    if 'Short Memo Body' not in [s.name for s in styles]:
        body_style = styles.add_style('Short Memo Body', WD_STYLE_TYPE.PARAGRAPH)
        body_font = body_style.font
        body_font.name = DOCUMENT_FONT
        body_font.size = Pt(9)
        body_style.paragraph_format.line_spacing = 1.1  
        body_style.paragraph_format.space_after = Pt(4) 

def add_short_header_footer(doc: Document, company_name: str, generation_date: str):
    """Add header and footer to the short document"""
    
    HEADER_FOOTER_FONT = 'Bangla Sangam MN'
    
    # Add header
    header = doc.sections[0].header
    header_para = header.paragraphs[0]
    header_para.text = f"CONFIDENTIAL - Initial IC Memo"
    header_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
def create_problem_solution_table(doc: Document, company_brief_content: str):
    """Create a problem/solution table for the short memo"""
    
    # Add spacing before table
    doc.add_paragraph()
    
    # Create table with 2 columns: Problem and Solution
    table = doc.add_table(rows=2, cols=2)
    table.style = 'Table Grid'
    table.alignment = WD_TABLE_ALIGNMENT.LEFT
    
    # Set column widths
    table.columns[0].width = Inches(3.0)  # Problem column
    table.columns[1].width = Inches(3.0)  # Solution column
    ...
    # Content row - extract problem and solution from company brief
    content_cells = table.rows[1].cells
    
    # Try to extract problem and solution from the company brief content
    # This is a simple extraction - you might want to make this more sophisticated
    problem_text = "Market inefficiency or pain point to be identified from company data"
    solution_text = "Company's solution and approach to be identified from company data"
    
    content_cells[0].text = problem_text
    content_cells[1].text = solution_text
...
def add_short_section_to_document(doc: Document, section_name: str, content: str, section_order: Dict[str, str]):
    """Add a section to the short Word document with compact formatting"""
    
    BODY_FONT = 'Bangla Sangam MN'
    BODY_SIZE = 9
    ...
    # Add section heading
    heading_para = doc.add_paragraph(section_title, style='Short Section Heading')
    
    # Add content with compact formatting
    para = doc.add_paragraph(style='Short Memo Body')
    add_formatted_text_to_paragraph(para, content, BODY_FONT, BODY_SIZE)