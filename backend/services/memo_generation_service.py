...
def generate_short_memo(
    ...
    print(f"Starting short memo generation for memo request {memo_request_id}")
    ...
    try:
        ...
        return {
            "status": "failed",
            "error": f"Failed to load short memo prompts: {str(e)}",
            "sections_completed": [],
            "sections_failed": []
        }
    ...
    for section in short_sections:
        if section in prompts:
            ...
            print(f"⚠️ Short memo prompt not found for section: {section}")
...
def compile_short_memo(db: Session, memo_request_id: int) -> str:
    ...
    # Define logical display order for short memo
    section_order = [
